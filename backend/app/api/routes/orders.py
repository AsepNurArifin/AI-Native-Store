...
router = APIRouter(prefix="/orders", tags=["orders"], dependencies=[Depends(require_staff_or_owner)])
...
async def get_order(order_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
...
async def complete_order(order_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
    order = await OrderService.get_with_items(db, order_id)
    if not order:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Order tidak ditemukan")
    try:
...
async def cancel_order(order_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
    order = await OrderService.get_with_items(db, order_id)
    if not order:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Order tidak ditemukan")
    try:
