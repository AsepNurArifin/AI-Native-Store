...
router = APIRouter(prefix="/products", tags=["products"], dependencies=[Depends(require_staff_or_owner)])
...
async def create_product(body: ProductCreate, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
...
async def get_product(product_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
    try:
...
async def update_product(product_id: str, body: ProductUpdate, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
    try:
        product = await ProductService.get_or_404(db, product_id)
    except ProductNotFound:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Produk tidak ditemukan")
...
async def delete_product(product_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_staff_or_owner)):
    try:
        product = await ProductService.get_or_404(db, product_id)
