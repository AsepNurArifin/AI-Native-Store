...
router = APIRouter(prefix="/customers", tags=["customers"], dependencies=[Depends(require_owner)])


@router.get("", response_model=list[CustomerOut])
async def list_customers(db: AsyncSession = Depends(get_session), _: User = Depends(require_owner)):
    rows = (await db.execute(select(Customer).order_by(Customer.registered_at.desc()).limit(200))).scalars().all()
    return list(rows)


@router.get("/{customer_id}", response_model=CustomerDetail)
async def get_customer(customer_id: str, db: AsyncSession = Depends(get_session), _: User = Depends(require_owner)):
