<template>
  <div class="space-y-4">
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>Tanya analis (bahasa natural)</CardTitle>
        </CardHeader>
        <CardContent>
          <form class="space-y-2" @submit.prevent="ask()">
            <textarea v-model="question" rows="2" placeholder="Produk apa yang paling laris bulan ini?" class="w-full rounded-md border border-stone-300 p-2 text-sm" />
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p v-if="salesLoading" class="text-sm text-stone-500">Memuat…</p>
          <template v-else-if="sales">
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr v-for="(row, i) in sales.data" :key="row.key" class="border-b border-stone-100">
                    ...
                    <td class="pr-2">
                      <div class="h-2 rounded bg-clay-600/80" :style="{ width: revenueBar(row.revenue) }" />
                    </td>
                  </tr>
                </tbody>
                <tfoot>
            ...
            <p v-else class="text-sm text-stone-500">Tidak ada penjualan pada rentang ini.</p>
          </template>
          <p v-else class="text-sm text-stone-500">Pilih rentang tanggal lalu muat.</p>
        </CardContent>
      </Card>
    </div>
    <div class="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>Risiko stockout</CardTitle>
        </CardHeader>
        <CardContent>
          <div class="mb-2 flex gap-2">
            <Input v-model.number="threshold" type="number" class="w-28" placeholder="7 hari" />
            <Button size="sm" variant="secondary" :loading="invLoading" @click="loadInv()">Muat</Button>
          </div>
          <p v-if="invLoading" class="text-sm text-stone-500">Memuat…</p>
          <template v-else-if="inv">
            <p class="mb-2 text-sm text-stone-500">
              Ambang batas: &le;{{ inv.threshold_days }} hari stok tersisa
              <Badge v-if="riskCount" variant="danger" class="ml-2">{{ riskCount }} produk berisiko</Badge>
            </p>
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr
                    ...
                    :class="row.stockout_risk ? 'bg-red-50 ' : ''"
                  >
                    <td class="py-2 pr-2">
                      {{ row.name }}
                    ...
                    <td><Badge :variant="row.stockout_risk ? 'danger' : 'muted'">{{ row.stockout_risk ? 'Berisiko' : 'Aman' }}</Badge></td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p v-else class="text-sm text-stone-500">Belum ada produk aktif.</p>
          </template>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          ...
          <p v-if="channelsLoading" class="mt-2 text-sm text-stone-500">Memuat…</p>
          <template v-else-if="channels">
            <p class="mt-2 mb-3 text-sm text-stone-500">
            ...
            <div v-if="channels.by_channel.length" class="space-y-3">
              <div v-for="row in channels.by_channel" :key="row.channel">
                <div class="mb-1 flex items-center justify-between text-sm">
                  ...
                  <span class="text-stone-500">{{ row.orders }} pesanan · {{ row.percent }}%</span>
                </div>
                <div class="h-3 w-full overflow-hidden rounded bg-stone-200">
                  <div class="h-full rounded bg-clay-600" :style="{ width: row.percent + '%' }" />
                </div>
              </div>
            </div>
